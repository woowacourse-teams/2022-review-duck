...
class TemplateTest {
    ...
    void createTemplate() {
        //when, then
        assertDoesNotThrow(() -> new Template("템플릿 제목", "템플릿 설명", List.of("질문1", "질문2")));
    }
    ...
    void notNullTitle(String templateTitle) {
        //when, then
        assertThatThrownBy(() -> new Template(templateTitle, "템플릿 설명", List.of()))
    ...
    void titleProperLength() {
        //when, then
        assertDoesNotThrow(() -> new Template("a".repeat(100), "템플릿 설명", List.of()));
    }
    ...
    void titleOverLength() {
        //when, then
        assertThatThrownBy(() -> new Template("a".repeat(101), "템플릿 설명", List.of()))
            .isInstanceOf(TemplateException.class)
    ...
    void notNullDescription(String templateDescription) {
        //when, then
        assertThatThrownBy(() -> new Template("templateTitle", templateDescription, List.of()))
            .isInstanceOf(TemplateException.class)
    ...
    void notNullQuestions(List<String> questions) {
        //when, then
        assertThatThrownBy(() -> new Template("질문목록", "템플릿 설명", questions))
            .isInstanceOf(TemplateException.class)
