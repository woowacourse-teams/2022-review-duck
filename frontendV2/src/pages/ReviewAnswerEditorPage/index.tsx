...
function ReviewAnswerEditorPage() {
  ...
  const redirectUri = searchParams.get('redirect') || `${PAGE_LIST.REVIEW_OVERVIEW}/${reviewFormCode}`;

  const { authorProfile, reviewForm, reviewAnswer, submitCreateAnswer, submitUpdateAnswer } = useAnswerEditorPage(
    reviewFormCode,
    reviewId,
  );

  const [questions, setQuestions] = useState<Question[]>(reviewId ? reviewAnswer.questions : reviewForm.questions);
  const {
  ...
  return (
    <>
      ...
      <Editor onSubmit={handleSubmit}>
        <Editor.TitleInput
          placeholder={reviewId ? reviewAnswer.info.reviewTitle : `${authorProfile?.nickname}의 회고`}
          title={reviewTitle}
